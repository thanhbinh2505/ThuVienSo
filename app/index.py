...
from app import app, dao, login, mail, oauth
...
def register_routes(app):
    ...
    def chi_tiet_sach(sach_id):
        ...
        if current_user.is_authenticated:
            dao.luu_lich_su_xem(current_user.id, sach_id)
        danh_sach_binh_luan = dao.get_binh_luan_sach(sach_id)
        sach_lien_quan = dao.get_sach_lien_quan(sach)
        return render_template('chi_tiet_sach.html', sach=sach, sach_lien_quan=sach_lien_quan, danh_sach_binh_luan=danh_sach_binh_luan)
    ...
    def api_tim_kiem_sach():
        ...
        ket_qua = dao.tim_kiem_sach(tu_khoa=data.get('q', ''), theloai_id=data.get('theloai_id'), page=data.get('page', 1), sort=data.get('sort', 'moi_nhat'))
    ...
    def register():
        ...
        if request.method == 'POST':
            ...
            if not data.get('email') and not data.get('sdt'):
                return render_template('register.html', err_msg='Vui lòng cung cấp ít nhất Email hoặc Số điện thoại!')
    ...
    def quen_mat_khau():
        if request.method == 'POST':
            ...
            if not dinh_danh:
                return render_template('quen_mat_khau.html', error='Vui lòng nhập Email hoặc số điện thoại!')
            if phuong_thuc == 'email':
                user = User.query.filter_by(email=dinh_danh).first()
                if not user:
                    return render_template('quen_mat_khau.html', error='Không tìm thấy Email này!')
            elif phuong_thuc == 'sms':
                user = User.query.filter_by(soDienThoai=dinh_danh).first()
                if not user:
                    return render_template('quen_mat_khau.html', error='Không tìm thấy số điện thoại này!')
            else:
                return render_template('quen_mat_khau.html', error='Phương thức không hợp lệ!')
            ...
            if phuong_thuc == 'email':
                try:
                    msg = Message(subject='Mã OTP đặt lại mật khẩu', sender=app.config['MAIL_USERNAME'], recipients=[user.email])
                    msg.body = f'\n    Mã OTP đặt lại mật khẩu của bạn là:\n\n    {otp}\n\n    Mã có hiệu lực trong 5 phút.\n    '
                    mail.send(msg)
                except Exception as e:
                    print('LỖI GỬI EMAIL:', e)
                    return render_template('quen_mat_khau.html', error='Không thể gửi Email!')
            elif phuong_thuc == 'sms':
                print(f'OTP gửi đến {user.soDienThoai}: {otp}')
            return redirect(url_for('xac_nhan_otp'))
    ...
    def xac_nhan_otp():
        ...
        if request.method == 'POST':
            ...
            otp_he_thong = session.get('reset_otp')
            if otp_nhap != otp_he_thong:
                error = 'Mã OTP không đúng!'
            else:
                user_id = session.get('reset_user_id')
                session.pop('reset_otp', None)
                return redirect(url_for('dat_lai_mat_khau', user_id=user_id))
        return render_template('xac_nhan_otp.html', error=error)
    ...
    def dat_lai_mat_khau(user_id):
        ...
        if request.method == 'POST':
            ...
            if not password or not confirm_password:
                return render_template('dat_lai_mat_khau.html', error='Vui lòng nhập đầy đủ mật khẩu!')
            if password != confirm_password:
                return render_template('dat_lai_mat_khau.html', error='Mật khẩu xác nhận không khớp!')
            success, message = dao.dat_lai_mat_khau(user_id, password)
            if success:
                return redirect(url_for('login_process'))
            return render_template('dat_lai_mat_khau.html', error=message)
    ...
    def login_google_callback():
        ...
        user, _ = dao.dang_nhap_hoac_tao_tai_khoan_oauth(provider=OAuthProvider.GOOGLE, oauth_id=userinfo.get('sub'), email=userinfo.get('email'), hoten=userinfo.get('name'), avatar=userinfo.get('picture'))
        login_user(user=user)
        return redirect('/')
    ...
    @anonymous_required
    def login_facebook():
        redirect_uri = url_for('login_facebook_callback', _external=True)
    ...
    def login_facebook_callback():
        ...
        user, _ = dao.dang_nhap_hoac_tao_tai_khoan_oauth(provider=OAuthProvider.FACEBOOK, oauth_id=userinfo.get('id'), email=userinfo.get('email'), hoten=userinfo.get('name'), avatar=picture_data.get('url'))
        login_user(user=user)
        return redirect('/')
    ...
    def api_danh_gia_sach(sach_id):
        ...
        success, message = dao.danh_gia_sach(current_user.id, sach_id, so_sao)
        if not success:
            return (jsonify({'success': False, 'message': message}), 400)
        sach = dao.get_sach_by_id(sach_id)
        return (jsonify({'success': True, 'message': message, 'diemDanhGiaTB': round(sach.diemDanhGiaTB or 0, 1), 'soLuotDanhGia': sach.soLuotDanhGia or 0}), 200)
    ...
    def api_them_binh_luan(sach_id):
        ...
        success, message, binh_luan = dao.them_binh_luan(current_user.id, sach_id, noi_dung)
        if not success:
            return (jsonify({'success': False, 'message': message}), 400)
        return (jsonify({'success': True, 'message': message, 'data': {'id': binh_luan.id, 'noiDung': binh_luan.noiDung, 'ngayTao': binh_luan.ngayTao.strftime('%d/%m/%Y %H:%M'), 'hoTen': binh_luan.user.hoTen}}), 200)

    @app.route('/api/binh-luan/<int:binh_luan_id>', methods=['DELETE'])
    def api_xoa_binh_luan(binh_luan_id):
        if not current_user.is_authenticated:
            return (jsonify({'success': False, 'message': 'Vui lòng đăng nhập!'}), 401)
        success, message = dao.xoa_binh_luan(current_user.id, binh_luan_id)
    ...
    def api_dang_ky_muon(sach_id):
        ...
        if current_user.role != UserRole.DOCGIA:
            return (jsonify({'success': False, 'message': 'Chỉ độc giả mới được đăng ký mượn sách!'}), 403)
        success, message = dao.dang_ky_muon_sach(current_user.id, sach_id)
        if not success:
            return (jsonify({'success': False, 'message': message}), 400)
    ...
    def api_gui_yeu_cau_gia_han(phieu_muon_id):
        if not current_user.is_authenticated:
            return (jsonify({'success': False, 'message': 'Vui lòng đăng nhập!'}), 401)
        if current_user.role != UserRole.DOCGIA:
            return (jsonify({'success': False, 'message': 'Chỉ độc giả mới được yêu cầu gia hạn!'}), 403)
        success, message = dao.gui_yeu_cau_gia_han(current_user.id, phieu_muon_id)
    ...
    def sach_dang_muon():
        ...
        danh_sach_dang_muon = dao.get_phieu_muon_dang_muon_cua_doc_gia(current_user.id)
        danh_sach_da_tra = dao.get_phieu_muon_da_tra_cua_doc_gia(current_user.id)
        danh_sach_qua_han = dao.get_phieu_muon_qua_han_cua_doc_gia(current_user.id)

        return render_template(
            'sach_dang_muon.html',
            danh_sach_dang_muon=danh_sach_dang_muon,
            danh_sach_da_tra=danh_sach_da_tra,
            danh_sach_qua_han=danh_sach_qua_han
        )
    ...
    def api_tra_sach(phieu_muon_id):
        if not current_user.is_authenticated:
            return (jsonify({'success': False, 'message': 'Vui lòng đăng nhập!'}), 401)
        if current_user.role != UserRole.DOCGIA:
            return (jsonify({'success': False, 'message': 'Chỉ độc giả mới được trả sách!'}), 403)
        success, message = dao.tra_sach(current_user.id, phieu_muon_id)
        return (jsonify({'success': success, 'message': message}), 200 if success else 400)

    @app.route('/api/sach/<int:sach_id>/yeu-thich', methods=['POST'])
    def api_toggle_yeu_thich(sach_id):
        ...
        if not sach:
            return (jsonify({'success': False, 'message': 'Không tìm thấy sách!'}), 404)
        da_yeu_thich, message = dao.toggle_yeu_thich(current_user.id, sach_id)
        return (jsonify({'success': True, 'message': message, 'da_yeu_thich': da_yeu_thich}), 200)

    @app.route('/yeu-thich')
    def danh_sach_yeu_thich():
        if not current_user.is_authenticated:
            return redirect(url_for('login_process'))
        danh_sach = dao.get_danh_sach_yeu_thich(current_user.id)
        return render_template('yeu_thich.html', danh_sach=danh_sach)
    ...
    def trang_thu_thu():
        ...
        return render_template('thuthu.html', danh_sach_cho_duyet=danh_sach_cho_duyet, danh_sach_cho_gia_han=danh_sach_cho_gia_han, danh_sach_dang_muon=danh_sach_dang_muon, so_yeu_cau_muon=len(danh_sach_cho_duyet), so_yeu_cau_gia_han=len(danh_sach_cho_gia_han), so_sach_dang_muon=len(danh_sach_dang_muon), tong_so_sach=Sach.query.count(), now=datetime.now())
    ...
    def du_doan_nhu_cau():
        ...
        return render_template('du_doan_nhu_cau.html', danh_sach_du_doan=danh_sach_du_doan)
    ...
    def quan_ly_sach():
        ...
        return render_template('quan_li_sach.html', danh_sach_sach=danh_sach_sach)
    ...
    def them_sach():
        ...
        if request.method == 'POST':
            ...
            if not ten_sach or not tac_gia:
                return render_template('them_sach.html', danh_sach_theloai=danh_sach_theloai, error='Vui lòng nhập tên sách và tác giả!')
            try:
                ...
                return render_template('them_sach.html', danh_sach_theloai=danh_sach_theloai, error='Dữ liệu số không hợp lệ!')
            success, message, sach = dao.them_sach(ten_sach=ten_sach, tac_gia=tac_gia, nha_xuat_ban=nha_xuat_ban, nam_xuat_ban=nam_xuat_ban, ngon_ngu=ngon_ngu, so_trang=so_trang, mo_ta=mo_ta, theloai_id=theloai_id, anh_bia=anh_bia, so_luong=so_luong)
            if success:
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                for row in ws.iter_rows(min_row=2, values_only=True):
                    ...
                    if not tac_gia:
                        continue
                    danh_sach_sach.append({'ten_sach': ten_sach, 'tac_gia': tac_gia, 'nha_xuat_ban': nha_xuat_ban, 'nam_xuat_ban': nam_xuat_ban, 'ngon_ngu': ngon_ngu, 'so_trang': so_trang, 'theloai_id': theloai_id, 'ten_the_loai': ten_the_loai, 'so_luong': so_luong, 'anh_bia': anh_bia, 'mo_ta': mo_ta})
    ...
    @app.route('/admin/sach/<int:sach_id>/sua', methods=['GET', 'POST'])
    @role_required([UserRole.ADMIN, UserRole.THUTHU])
    def sua_sach(sach_id):
        ...
        if request.method == 'POST':
            ...
            if file and file.filename != '':
                ...
                anh_bia = anh_bia_url
            if not ten_sach or not tac_gia:
                return render_template('sua_sach.html', sach=sach, danh_sach_theloai=danh_sach_theloai, error='Vui lòng nhập tên sách và tác giả!')
            try:
                nam_xuat_ban = int(nam_xuat_ban) if nam_xuat_ban else None
                so_trang = int(so_trang) if so_trang else None
                theloai_id = int(theloai_id) if theloai_id else None
                ...
                return render_template('sua_sach.html', sach=sach, danh_sach_theloai=danh_sach_theloai, error='Dữ liệu số không hợp lệ!')
            success, message = dao.cap_nhat_sach(sach_id=sach_id, ten_sach=ten_sach, tac_gia=tac_gia, nha_xuat_ban=nha_xuat_ban, nam_xuat_ban=nam_xuat_ban, ngon_ngu=ngon_ngu, so_trang=so_trang, mo_ta=mo_ta, theloai_id=theloai_id, so_luong=so_luong, anh_bia=anh_bia)
            if success:
                return redirect(url_for('quan_ly_sach'))
            return render_template('sua_sach.html', sach=sach, danh_sach_theloai=danh_sach_theloai, error=message)
        return render_template('sua_sach.html', sach=sach, danh_sach_theloai=danh_sach_theloai)

    @app.route('/admin/sach/<int:sach_id>/xoa', methods=['POST'])
    @role_required([UserRole.ADMIN, UserRole.THUTHU])
    ...
    def quan_ly_the_loai():
        ...
        return render_template('quan_li_the_loai.html', danh_sach_the_loai=danh_sach_the_loai)
    ...
    def them_the_loai():
        if request.method == 'POST':
            ...
            mo_ta = request.form.get('mo_ta', '').strip()
            success, message = dao.them_theloai(ten_the_loai, mo_ta)
            if success:
                return redirect(url_for('quan_ly_the_loai'))
            return render_template('them_the_loai.html', message=message)
    ...
    def sua_the_loai(theloai_id):
        ...
        if request.method == 'POST':
            ten_the_loai = request.form.get('ten_the_loai', '').strip()
            mo_ta = request.form.get('mo_ta', '').strip()
            success, message = dao.cap_nhat_theloai(theloai_id, ten_the_loai, mo_ta)
            if success:
                return redirect(url_for('quan_ly_the_loai'))
            return render_template('sua_the_loai.html', the_loai=the_loai, message=message)
        return render_template('sua_the_loai.html', the_loai=the_loai)
    ...
    def quan_ly_nguoi_dung():
        ...
        return render_template('quan_li_nguoi_dung.html', danh_sach_user=danh_sach_user)

    @app.route('/admin/nguoi-dung/<int:user_id>/khoa', methods=['POST'])
    @role_required(UserRole.ADMIN)
    def khoa_nguoi_dung(user_id):
        dao.khoa_user(user_id)
        return redirect(url_for('quan_ly_nguoi_dung'))

    @app.route('/admin/nguoi-dung/<int:user_id>/mo-khoa', methods=['POST'])
    @role_required(UserRole.ADMIN)
    def mo_khoa_nguoi_dung(user_id):
        dao.mo_khoa_user(user_id)
        return redirect(url_for('quan_ly_nguoi_dung'))

    @app.route('/admin/nguoi-dung/<int:user_id>/xoa', methods=['POST'])
    @role_required(UserRole.ADMIN)
    ...
    def api_tim_kiem_ngu_nghia():
        ...
        if not query:
            return jsonify([])
        danh_sach_sach = dao.tim_kiem_ngu_nghia(query)
        ket_qua = []
        for sach in danh_sach_sach:
            sach_dict = sach.to_dict()
            sach_dict['do_phu_hop'] = getattr(sach, 'do_phu_hop', 0)
            ket_qua.append(sach_dict)

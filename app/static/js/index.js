let state = {
    q: "",
    theloai_id: "",
    sort: "moi_nhat",
    page: 1,
};

let debounceTimer = null;
let searchMode = "normal";

// =====================================
// LẤY PHẦN TỬ HTML
// =====================================

const normalSearch = document.getElementById("normalSearch");
const semanticSearch = document.getElementById("semanticSearch");

const searchInput = document.getElementById("searchInput");
const theloaiSelect = document.getElementById("theloaiSelect");
const sortSelect = document.getElementById("sortSelect");
const btnSearch = document.getElementById("btnSearch");
const chips = document.querySelectorAll(".chip-theloai");


// =====================================
// CHUYỂN SANG TÌM KIẾM THƯỜNG
// =====================================

normalSearch.addEventListener("click", function () {

    searchMode = "normal";

    normalSearch.classList.add(
        "bg-primary",
        "text-white"
    );

    semanticSearch.classList.remove(
        "bg-primary",
        "text-white"
    );

    state.q = searchInput.value.trim();
    state.page = 1;

    timKiemSach();

});


// =====================================
// CHUYỂN SANG TÌM KIẾM NGỮ NGHĨA
// =====================================

semanticSearch.addEventListener("click", function () {

    searchMode = "semantic";

    semanticSearch.classList.add(
        "bg-primary",
        "text-white"
    );

    normalSearch.classList.remove(
        "bg-primary",
        "text-white"
    );

    state.q = searchInput.value.trim();
    state.page = 1;

    timKiemSach();

});


// =====================================
// CHỐNG HTML XẤU
// =====================================

function escapeHtml(str) {

    const div = document.createElement("div");

    div.innerText = str ?? "";

    return div.innerHTML;

}


// =====================================
// CHỌN CHIP THỂ LOẠI
// =====================================

function setActiveChip(id) {

    chips.forEach(chip => {

        const active =
            (chip.dataset.id || "") === (id || "");

        chip.classList.toggle(
            "active",
            active
        );

        chip.classList.toggle(
            "bg-primary",
            active
        );

        chip.classList.toggle(
            "text-white",
            active
        );

        chip.classList.toggle(
            "border-primary",
            active
        );

        chip.classList.toggle(
            "border-gray-200",
            !active
        );

        chip.classList.toggle(
            "text-slate-600",
            !active
        );

    });

}


// =====================================
// HIỂN THỊ CARD SÁCH
// =====================================

function renderBookCard(s) {

    const badgeClass =
        s.soLuongConLai > 0
            ? "bg-green-100 text-green-600"
            : "bg-red-100 text-red-500";

    const badgeText =
        s.soLuongConLai > 0
            ? "Còn sách"
            : "Hết sách";

    return `
        <a href="/sach/${s.id}"
           class="group bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300">

            <div class="aspect-[3/4] overflow-hidden bg-slate-100">

                <img
                    src="${s.anhBia || "/static/image/default.png"}"
                    alt="${escapeHtml(s.tenSach)}"
                    class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                    onerror="this.src='/static/image/default.png'"
                >

            </div>

            <div class="p-4">

                <h3 class="font-bold text-slate-900 line-clamp-2 mb-1 group-hover:text-primary transition-colors">

                    ${escapeHtml(s.tenSach)}

                </h3>

                <p class="text-sm text-slate-500 mb-2 truncate">

                    ${escapeHtml(s.tacGia)}

                </p>

                <div class="flex items-center justify-between text-xs">

                    <span class="flex items-center gap-1 text-amber-500 font-semibold">

                        <i
                            data-lucide="star"
                            class="w-3.5 h-3.5 fill-current"
                        ></i>

                        ${s.diemDanhGiaTB ?? 0}

                    </span>

                    <span class="px-2 py-1 rounded-full font-semibold ${badgeClass}">

                        ${badgeText}

                    </span>

                </div>

            </div>

        </a>
    `;

}


// =====================================
// PHÂN TRANG
// =====================================

function renderPagination(data) {

    const container =
        document.getElementById("pagination");

    container.innerHTML = "";

    if (!data.total_pages || data.total_pages <= 1) {
        return;
    }

    for (
        let i = 1;
        i <= data.total_pages;
        i++
    ) {

        const btn =
            document.createElement("button");

        btn.innerText = i;

        const active =
            i === data.page;

        btn.className =
            `w-10 h-10 rounded-lg text-sm font-semibold transition-colors ${
                active
                    ? "bg-primary text-white"
                    : "bg-white border border-gray-200 text-slate-600 hover:border-primary hover:text-primary"
            }`;

        btn.addEventListener(
            "click",
            () => {

                state.page = i;

                timKiemSach();

                const section =
                    document.getElementById("the-loai");

                if (section) {

                    window.scrollTo({
                        top:
                            section.offsetTop - 100,
                        behavior:
                            "smooth"
                    });

                }

            }
        );

        container.appendChild(btn);

    }

}


// =====================================
// HÀM TÌM KIẾM CHÍNH
// =====================================

async function timKiemSach() {

    const grid =
        document.getElementById("bookGrid");

    const loading =
        document.getElementById("loadingBooks");

    const empty =
        document.getElementById("emptyState");

    const pagination =
        document.getElementById("pagination");


    if (!grid || !loading || !empty) {

        console.error(
            "Không tìm thấy bookGrid, loadingBooks hoặc emptyState"
        );

        return;

    }


    loading.classList.remove("hidden");

    grid.classList.add("hidden");

    empty.classList.add("hidden");


    try {

        // =====================================
        // TÌM KIẾM NGỮ NGHĨA
        // =====================================

        if (searchMode === "semantic") {

            if (!state.q) {

                grid.innerHTML = "";

                empty.classList.remove(
                    "hidden"
                );

                if (pagination) {
                    pagination.innerHTML = "";
                }

                return;

            }


            const apiUrl =
                `/api/tim-kiem-ngu-nghia?q=${encodeURIComponent(
                    state.q
                )}`;


            const res =
                await fetch(apiUrl);


            if (!res.ok) {

                throw new Error(
                    `Lỗi API tìm kiếm ngữ nghĩa: ${res.status}`
                );

            }


            const items =
                await res.json();


            console.log(
                "Kết quả ngữ nghĩa:",
                items
            );


            document.getElementById(
                "resultCount"
            ).innerText =
                items.length > 0
                    ? `(${items.length} kết quả)`
                    : "";


            document.getElementById(
                "resultTitle"
            ).innerText =
                `Kết quả tìm kiếm ngữ nghĩa: "${state.q}"`;


            if (
                !Array.isArray(items) ||
                items.length === 0
            ) {

                grid.innerHTML = "";

                empty.classList.remove(
                    "hidden"
                );

            } else {

                grid.innerHTML =
                    items
                        .map(renderBookCard)
                        .join("");

                grid.classList.remove(
                    "hidden"
                );

            }


            if (pagination) {

                pagination.innerHTML = "";

            }

        }


        // =====================================
        // TÌM KIẾM THƯỜNG
        // =====================================

        else {

            const params =
                new URLSearchParams({
                    q: state.q,
                    sort: state.sort,
                    page: state.page
                });


            if (state.theloai_id) {

                params.set(
                    "theloai_id",
                    state.theloai_id
                );

            }


            const apiUrl =
                `/api/sach?${params.toString()}`;


            const res =
                await fetch(apiUrl);


            if (!res.ok) {

                throw new Error(
                    `Lỗi API tìm kiếm sách: ${res.status}`
                );

            }


            const data =
                await res.json();


            console.log(
                "Kết quả tìm kiếm thường:",
                data
            );


            document.getElementById(
                "resultCount"
            ).innerText =
                data.total
                    ? `(${data.total} kết quả)`
                    : "";


            document.getElementById(
                "resultTitle"
            ).innerText =
                state.q
                    ? `Kết quả tìm kiếm: "${state.q}"`
                    : "Tất cả sách";


            if (
                !data.items ||
                data.items.length === 0
            ) {

                grid.innerHTML = "";

                empty.classList.remove(
                    "hidden"
                );

            } else {

                grid.innerHTML =
                    data.items
                        .map(renderBookCard)
                        .join("");

                grid.classList.remove(
                    "hidden"
                );

            }


            renderPagination(data);

        }


        // =====================================
        // TẠO LẠI ICON
        // =====================================

        if (typeof lucide !== "undefined") {

            lucide.createIcons();

        }


    } catch (e) {

        console.error(
            "Lỗi tìm kiếm:",
            e
        );


        grid.innerHTML = `
            <p class="col-span-full text-center text-red-500 py-10">

                Lỗi tải dữ liệu sách.

                <br>

                <span class="text-sm">
                    ${escapeHtml(e.message)}
                </span>

            </p>
        `;


        grid.classList.remove(
            "hidden"
        );


    } finally {

        loading.classList.add(
            "hidden"
        );

    }

}


// =====================================
// NHẬP TỪ KHÓA
// =====================================

searchInput.addEventListener(
    "input",
    (e) => {

        clearTimeout(
            debounceTimer
        );


        debounceTimer =
            setTimeout(
                () => {

                    state.q =
                        e.target.value.trim();

                    state.page = 1;

                    timKiemSach();

                },
                400
            );

    }
);


// =====================================
// BẤM NÚT TÌM KIẾM
// =====================================

btnSearch.addEventListener(
    "click",
    () => {

        state.q =
            searchInput.value.trim();

        state.page = 1;

        timKiemSach();

    }
);


// =====================================
// BẤM ENTER
// =====================================

searchInput.addEventListener(
    "keydown",
    (e) => {

        if (e.key === "Enter") {

            e.preventDefault();

            state.q =
                searchInput.value.trim();

            state.page = 1;

            timKiemSach();

        }

    }
);


// =====================================
// CHỌN THỂ LOẠI
// =====================================

theloaiSelect.addEventListener(
    "change",
    (e) => {

        state.theloai_id =
            e.target.value;

        state.page = 1;

        setActiveChip(
            e.target.value
        );

        timKiemSach();

    }
);


// =====================================
// SẮP XẾP
// =====================================

sortSelect.addEventListener(
    "change",
    (e) => {

        state.sort =
            e.target.value;

        state.page = 1;

        timKiemSach();

    }
);


// =====================================
// CLICK CHIP THỂ LOẠI
// =====================================

chips.forEach(
    chip => {

        chip.addEventListener(
            "click",
            () => {

                state.theloai_id =
                    chip.dataset.id || "";

                theloaiSelect.value =
                    state.theloai_id;

                state.page = 1;

                setActiveChip(
                    state.theloai_id
                );

                timKiemSach();

            }
        );

    }
);